...
class invoice extends CI_Controller 
{
	...
	public function displayInvoices()
	{
	    if($this->checksession()==1)
		{
	     ...
	     $userdata["title"]="View Invoices";
	     $userdata["invoices"]=$this->invoices_model->getalldata("id,ito,ifrom,other_details","invoices");
		 $getid= $this->session->userdata('adminid');
		 $data=array('id'=>$getid);
		 $userdata["invoices"]=$this->user_model->get_info('invoices',$data);
		 $userdata["ito"]=$userdata["userinfo"][0]->ito;
		 $this->load->view('admin/inneradmin/header_layout',$userdata);
	...
	public function addInvoice()
	{
	    if($this->checksession()==1)
		{
		 ...
		 if($_POST)
		 {
		   ...
		   {
		   $this->invoices_model->insert_data("invoices",array("ito"=>$this->input->post('ito'),"ifrom"=>$this->input->post('ifrom'),
		   "softcopy"=>$this->input->post('softcopy'),
		 "other_details"=>$this->input->post('odetails')));
		   redirect('/admin/displayinvoices','refresh');		 
		   }
		 }
		 $this->load->view('admin/inneradmin/header_layout',$userdata);
	     $this->load->view('admin/inneradmin/addInvoice',$userdata);
		 $this->load->view('admin/inneradmin/footer_layout');
		 }
		 else
		 {
